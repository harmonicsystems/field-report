...
- `index.html` — minimal landing page introducing kinderhook.json
- `audit.html` — Vol. I, single-file static report for Kinderhook (the working notes)
- `ai-audit.html` — the AI surface audit page. GENERATED by
  `tools/build-audit-page.mjs` from `schema/audit/`; never hand-edit it.
- `kinderhook.json` — the canonical JSON-LD export, built by `tools/build-jsonld.mjs`
- `schema/` — documented ontology, namespaced extensions, hand-curated places
- `schema/audit/` — AI surface audit corpus: fixed query set (`queries.json`),
  monthly observation cycles (`cycles/YYYY-MM.json`), capture template.
  Collection is MANUAL by design — no SERP scraping, no headless browsers,
  no new fetchers for this module. Validated by `tools/validate-audit.mjs`
  (both builders refuse to run on an invalid corpus). Monthly workflow is
  in README.md; modeling calls are in `schema/ontology.md`.
